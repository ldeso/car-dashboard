...
/// \brief Classe jauge d'escence, permettant la création d'une jauge paramétrables .
/// \details Classe héritée de objet_virtuel. Utilisée pour l'affichage de la jauge d'escence ou de toute autre jauge du même type. L'appel au constructeur permet d'initialiser la totalité des paramètres, et la fonction paint permet l'affichage.
///
...
/// \param param_x position horizontale du coin en haut à gauche du rectangle définissant la gauge
/// \param param_y position verticale coin en haut à gauche du rectangle définissant la gauge
...
/// \details Cette fonction construit un compteur en plusieurs étapes : création du cadre de la jauge, des rectangles indiquant le niveau d'escence, du E et du F ainsi que l'icone/voyant d'escence.
/// \param painter
///
...
void FuelGauge_lea::paint(QPainter *painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    ...
    /// \details Les rectangles sont obtenus avec un drawRect d'un vecteur(pour automatiser le dessin avec un interval constant), ici 10 rectangles sont déssinées representant chacun 10% du niveau d'escence.
    ///
    ...
    ///
    /// \brief Création de l'icone d'escence de couleur blanche et devenant rouge l'orsqu'on passe en dessous de 25% d'escence.
    /// \details Les voyants sont obtenus via un drawPixmap dans une boucle if pour changer le pixmap en fonction de la value d'escence.
    ///
